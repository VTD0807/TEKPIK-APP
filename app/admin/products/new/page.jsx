    ...
    return (
        <div className="max-w-3xl space-y-6 mb-28">
            ...
            <form onSubmit={handleSubmit} className="bg-white border border-slate-100 rounded-xl p-6 shadow-sm space-y-5">
                ...
                {/* Category */}
                <div className="flex flex-col gap-1">
                    <label className="text-xs font-medium text-slate-500">Category <span className="text-slate-700">*</span></label>
                    <select value={form.categoryId} onChange={e => set('categoryId', e.target.value)} required
                        className="border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-slate-400 bg-white">
                    ...
                    {categories.length === 0 && (
                        <p className="text-[11px] text-slate-800">No categories yet — <Link href="/admin/categories" className="underline">add one first</Link></p>
                    )}
                </div>
                ...
                <div className="flex items-center gap-3 pt-2 border-t border-slate-100">
                    <button type="submit" disabled={saving}
                        ...
                        {saving ? (
                            <><ArrowRepeat className="animate-spin" size={14} /> Saving...</>
                        ) : (
