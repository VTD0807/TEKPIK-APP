...
export async function getProductDetail(productId) {
    ...
    return getCachedSWR(CK.productDetail(productId), TTL.PRODUCT_DETAIL, TTL.PRODUCT_STALE, async () => {
        ...
        if (!productSnap.exists) {
            // Try slug lookup
            const slugSnap = await dbAdmin.collection('products')
                .where('slug', '==', productId)
                .limit(1)
                .get()
            if (slugSnap.empty) return null
            return _hydrateProduct(slugSnap.docs[0])
        }
        return _hydrateProduct(productSnap)
    })
}

/** Hydrate a product doc with category, AI analysis, and reviews in parallel. */
async function _hydrateProduct(doc) {
    ...
    const [categoriesMap, aiResult, reviewsSnap] = await Promise.all([
        getCategoriesMap(),
        dbAdmin.collection('ai_analysis')
...
export async function getProductFeatureVectors(productIds = []) {
    if (!dbAdmin || !productIds.length) return {}

    return getCachedSWR(CK.featureVectors(), TTL.FEATURE_VECTORS, TTL.FEATURE_STALE, async () => {
        const refs = productIds.map(id => dbAdmin.collection('analytics_product_feature_vectors').doc(id))
        const featureMap = {}
...
export async function getProductsByIds(productIds = []) {
    ...
    const productsMap = {}
    const refs = uniqueIds.map(pid => dbAdmin.collection('products').doc(pid))
