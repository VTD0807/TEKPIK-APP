...
/**
 * Get global application settings with caching.
 */
export async function getGlobalSettings() {
    return getCachedSWR('db:global_settings:v1', 5 * 60 * 1000, 3 * 60 * 1000, async () => {
        const prodDb = await getProductionDb()
        if (!prodDb) return {}
        try {
            const doc = await prodDb.collection('settings').doc('general').get()
...
/**
 * Get downtime/traffic-manager settings from DB-4 (Workspace).
 * Includes: maintenanceMode, blockBuyNow, blockSignups, blockedRoutes, fallbackUrl
 */
export async function getDowntimeSettings() {
    return getCachedSWR('db:downtime_settings:v1', 30 * 1000, 15 * 1000, async () => {
        if (!dbWorkspace) return {}
        try {
            const doc = await dbWorkspace.collection('settings').doc('downtime').get()
            return doc.exists ? doc.data() : {}
        } catch (e) {
            console.error('Failed to fetch downtime settings:', e)
            return {}
        }
    })
}

export function invalidateDowntimeCache() {
    invalidateCache('db:downtime_settings:v1')
}
...
export async function getAdminCatalog() {
    ...
    if (!db) return { products: [], categoriesMap: {}, aiMap: {} }
...
export async function getAiAnalysisMap() {
    ...
    return getCachedSWR(CK.aiAnalysisMap(), TTL.AI_ANALYSIS, TTL.AI_ANALYSIS_STALE, async () => {
        const snap = await db.collection('ai_analysis').get()
        const map = {}
        snap.forEach(doc => {
            const data = doc.data()
...
async function _hydrateProduct(doc, prodDb) {
    ...
    const [categoriesMap, aiResult, reviewsSnap] = await Promise.all([
        ...
        prodDb.collection('reviews')
            .where('productId', '==', product.id)
            .get(), // Removed .where('isApproved', '==', true) to avoid composite index error
    ])
    ...
    product.reviews = reviewsSnap.docs
        .filter(r => r.data().isApproved === true) // In-memory filter
        .map(r => ({
            id: r.id,
            ...r.data(),
        }))

    return sanitizeFirestoreData(product)
}
...
export async function getProductReviews(productId) {
    ...
    return getCachedSWR(CK.productReviews(productId), TTL.REVIEWS, TTL.REVIEWS_STALE, async () => {
        const snapshot = await prodDb.collection('reviews')
            ...
            .get() // Removed .where and .orderBy to avoid composite index errors on failover DB

        let reviews = snapshot.docs
            .filter(doc => doc.data().isApproved === true) // In-memory filter
            .map(doc => ({
                id: doc.id,
                ...doc.data(),
            }))

        reviews = sanitizeFirestoreData(reviews)
