...
export async function reportDatabaseQuotaError(error) {
    // 1. Quick memory check (helps within the same lambda instance)
    const now = Date.now()
    if (now - memoryLastAlertTime < 30 * 60 * 1000) return
    memoryLastAlertTime = now

    try {
        const { dbUsers, dbWorkspace } = await import('@/lib/firebase-admin')
        if (!dbUsers || !dbWorkspace) return

        // 2. Distributed Lock via Firestore Transaction (prevents concurrent lambda spam)
        const lockRef = dbWorkspace.collection('settings').doc('quota_alert_lock')
        let shouldSend = false

        try {
            await dbWorkspace.runTransaction(async (t) => {
                const doc = await t.get(lockRef)
                if (doc.exists) {
                    const lastSent = doc.data().lastSentAt || 0
                    if (now - lastSent < 30 * 60 * 1000) {
                        return // Within cooldown
                    }
                }
                t.set(lockRef, { lastSentAt: now }, { merge: true })
                shouldSend = true
            })
        } catch (lockErr) {
            console.error('[mailer] Failed to acquire quota alert lock', lockErr)
            return // Fail safe: don't send if we can't verify lock
        }

        if (!shouldSend) return

        // 3. Find all admins and employees to notify
