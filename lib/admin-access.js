import { authUsers, dbUsers } from '@/lib/firebase-admin'
...
export async function getAccessContext(req) {
    if (!authUsers || !dbUsers) {
        return { ok: false, status: 500, error: 'Auth/Users DB not initialized' }
    }
    ...
    try {
        ...
        const decoded = await authUsers.verifyIdToken(token)
        const userSnap = await dbUsers.collection('users').doc(decoded.uid).get()
