...
const parseFirstValidAmount = (...values) => {
    for (const value of values) {
        const parsed = parseAmount(value) ?? parseAmountLoose(value)
...
const extractFromJsonLd = ($) => {
    ...
    const brandName = typeof brandRaw === 'string'
        ? brandRaw
        : (typeof brandRaw?.name === 'string' ? brandRaw.name : '')

    return {
        title: firstNonEmpty(productNode.name),
        brand: firstNonEmpty(brandName),
...
export async function scrapeAmazonProduct(sourceUrl) {
    ...
    const currentPriceText = firstNonEmpty(
        $('#corePriceDisplay_desktop_feature_div .a-price:not(.a-text-price) .a-offscreen').first().text(),
        $('#corePriceDisplay_desktop_feature_div .a-offscreen').first().text(),
        $('#corePrice_feature_div .a-price:not(.a-text-price) .a-offscreen').first().text(),
        $('#corePrice_feature_div .a-offscreen').first().text(),
        $('#apex_desktop .a-price:not(.a-text-price) .a-offscreen').first().text(),
    ...
    // Brand: try dedicated brand row in tech specs / product details table, then byline, then JSON-LD
    const brand = firstNonEmpty(
        // Product details table row
        $('#productDetails_techSpec_section_1 tr th:contains("Brand") + td').first().text(),
        $('#productDetails_detailBullets_sections1 tr th:contains("Brand") + td').first().text(),
        // Detail bullets list
        $('span.a-text-bold:contains("Brand")').first().parent().text().replace(/Brand\s*:?\s*/i, ''),
        // Byline / brand link
        $('#bylineInfo').first().text(),
        $('#brand').first().text(),
        $('a#bylineInfo').first().text(),
        // Regex fallback from raw HTML
        (() => {
            const m = String(html).match(/"brand"\s*:\s*"([^"]{1,80})"/i)
            return m?.[1] || ''
        })(),
        jsonLd.brand
    )
    ...
    const parsedPrice = parseFirstValidAmount(
        currentPriceText,
        extractPriceByRegex(html, ['priceToPay', 'dealPrice', 'ourPrice', 'priceAmount', 'currentPrice'])
    )
    const parsedOriginalPrice = parseFirstValidAmount(
        originalPriceText,
        extractPriceByRegex(html, ['listPrice', 'priceBeforeDeal', 'wasPrice'])
    )
