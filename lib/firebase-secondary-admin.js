...
const parseJsonFile = (absolutePath) => {
...
const buildSecondaryServiceAccount = () => {
    ...
    const fileCandidates = [
        process.env.SECONDARY_FIREBASE_SERVICE_ACCOUNT_FILE,
        path.join(process.cwd(), 'secondary-firebase-service-account.json'),
        path.join(process.cwd(), 'pricetrack-tekpik-firebase-adminsdk.json'),
    ].filter(Boolean)

    for (const candidate of fileCandidates) {
        const fromFile = parseJsonFile(candidate)
        if (fromFile) {
            return { source: `file:${candidate}`, credentials: fromFile }
        }
    }
    ...
    const fromPrimaryServiceAccount = parseMaybeJson(process.env.FIREBASE_SERVICE_ACCOUNT)
    if (fromPrimaryServiceAccount) {
        return {
            source: 'FIREBASE_SERVICE_ACCOUNT fallback',
            credentials: {
                ...fromPrimaryServiceAccount,
                project_id: process.env.SECONDARY_FIREBASE_PROJECT_ID || fromPrimaryServiceAccount.project_id,
                projectId: process.env.SECONDARY_FIREBASE_PROJECT_ID || fromPrimaryServiceAccount.projectId || fromPrimaryServiceAccount.project_id,
            },
        }
    }
