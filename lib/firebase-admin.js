...
export const authUsers = usersApp ? usersApp.auth() : (primaryApp ? primaryApp.auth() : null)
...
export async function getProductionDb() {
    ...
    try {
        ...
        routerCacheExpiry = Date.now() + 1000 * 30 // 30 second cache for fast failover
...
/**
 * Get the best available DB for admin product/content operations.
 * Uses the production DB router (with failover to secondary) so admin
 * doesn't break when primary quota is exhausted.
 * Falls back to dbAdmin if router is unavailable.
 */
export async function getAdminDb() {
    const prodDb = await getProductionDb()
    return prodDb || dbAdmin
}

/**
 * Get the DB for analytics reads (devices, visitors, etc).
 * Routes to dbUsers which is isolated from product quota.
 */
export function getAnalyticsDb() {
    return dbUsers || dbAdmin
}
