...
const initializeAppInstance = (appName, jsonEnvKey, base64EnvKey) => {
    try {
        ...
        // Fallback to legacy single-project keys if the specific 3-DB keys aren't provided yet
        if (!creds && jsonEnvKey !== 'FIREBASE_SERVICE_ACCOUNT') {
            creds = parseMaybeJson(process.env.FIREBASE_SERVICE_ACCOUNT)
        }
...
if (!primaryApp) primaryApp = initializeAppInstance('primary', 'FIREBASE_PRIMARY_SERVICE_ACCOUNT', 'FIREBASE_PRIMARY_SERVICE_ACCOUNT_BASE64')
if (!secondaryApp) secondaryApp = initializeAppInstance('secondary', 'FIREBASE_SECONDARY_SERVICE_ACCOUNT', 'FIREBASE_SECONDARY_SERVICE_ACCOUNT_BASE64')
if (!usersApp) usersApp = initializeAppInstance('users', 'FIREBASE_USERS_SERVICE_ACCOUNT', 'FIREBASE_USERS_SERVICE_ACCOUNT_BASE64')
if (!workspaceApp) workspaceApp = initializeAppInstance('workspace', 'FIREBASE_WORKSPACE_SERVICE_ACCOUNT', 'FIREBASE_WORKSPACE_SERVICE_ACCOUNT_BASE64')
...
export const firebaseAdminStatus = {
    ready: !!dbAdmin,
    multiDbEnabled: !!(primaryApp && secondaryApp && workspaceApp),
}
