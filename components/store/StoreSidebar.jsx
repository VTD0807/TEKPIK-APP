...
import { HouseDoor, List, PencilSquare, PlusSquare } from 'react-bootstrap-icons'
...
const StoreSidebar = ({storeInfo}) => {
    ...
    const sidebarLinks = [
        { name: 'Dashboard', href: '/store', icon: HouseDoor },
        { name: 'Add Product', href: '/store/add-product', icon: PlusSquare },
        { name: 'Manage Product', href: '/store/manage-product', icon: PencilSquare },
            ...
            <div className="max-sm:mt-6">
                {
                    sidebarLinks.map((link, index) => (
                        <Link key={index} href={link.href} className={`relative flex items-center gap-3 text-slate-500 hover:bg-slate-50 p-2.5 transition ${pathname === link.href && 'bg-slate-100 sm:text-slate-600'}`}>
                            ...
                            {pathname === link.href && <span className="absolute bg-black right-0 top-1.5 bottom-1.5 w-1 sm:w-1.5 rounded-l"></span>}
                        </Link>
                    ))
                }
            </div>
