...
const Hero = ({ settings = {}, categories = [] }) => {
    ...
    return (
        <div className='mx-6'>
            <div className='flex max-xl:flex-col gap-6 md:gap-8 max-w-7xl mx-auto my-10 min-h-[400px]'>
                
                {/* Main Left Banner */}
                <Link href={main?.link || "/shop"} className='relative flex-1 flex flex-col bg-[#baf4cb] rounded-[3.25rem_2.25rem_3.75rem_2.25rem] overflow-hidden group hover:ring-2 ring-[#aaeac3] ring-offset-2 transition-all duration-300'>
                    {main?.imageUrl ? (
                        <img 
                            src={main.imageUrl} 
                        ...
                        <div className='p-8 sm:p-14 relative z-10 flex flex-col justify-center h-full'>
                            ...
                            <h2 className='text-3xl sm:text-5xl leading-[1.2] my-3 font-serif font-semibold text-slate-900 max-w-xs sm:max-w-sm'>
                                {main?.title || "Gadgets you'll love. Prices you'll trust."}
                            </h2>
                            <div className='mt-8'>
                                <span className='inline-block bg-slate-900 text-white text-[11px] py-4 px-10 rounded-[1.25rem] shadow-xl hover:bg-slate-800 transition font-mono uppercase tracking-[0.3em]'>
                                    LEARN MORE
                                </span>
                            </div>
                        </div>
                ...
                <div className='flex flex-col gap-6 md:gap-8 w-full xl:max-w-[400px]'>
                    
                    {/* Top Right Box */}
                    <Link href={side1?.link || "/shop"} className='flex-1 flex flex-col justify-center w-full bg-[#fedcc0] rounded-[2.5rem_3.25rem_2rem_3.5rem] p-8 group relative overflow-hidden hover:ring-2 ring-[#ffd8b8] ring-offset-2 transition-all duration-300 min-h-[190px]'>
                        {side1?.imageUrl ? (
                            ...
                            <div className="relative z-10 space-y-3">
                                ...
                                <p className='text-2xl sm:text-3xl font-serif font-semibold text-slate-900 max-w-[12rem]'>{side1?.title || "Best products"}</p>
                    ...
                    {/* Bottom Right Box */}
                    <Link href={side2?.link || "/shop"} className='flex-1 flex flex-col justify-center w-full bg-[#bfddfe] rounded-[3rem_2rem_3.25rem_2.5rem] p-8 group relative overflow-hidden hover:ring-2 ring-[#b6d8fd] ring-offset-2 transition-all duration-300 min-h-[190px]'>
                        {side2?.imageUrl ? (
                            ...
                            <div className="relative z-10 space-y-3">
                                ...
                                <p className='text-2xl sm:text-3xl font-serif font-semibold text-slate-900 max-w-[12rem]'>{side2?.title || "20% discounts"}</p>
                                <p className='flex items-center gap-1 text-[11px] text-slate-700 font-mono uppercase tracking-[0.25em]'>View more <ArrowRight className='group-hover:ml-1 transition-all' size={14} /></p>
                            </div>
                        )}
                    </Link>
