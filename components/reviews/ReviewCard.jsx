...
export default function ReviewCard({ review, onClick, currentUserId, onLike, onDislike, onDelete }) {
    ...
    return (
        <div
            onClick={onClick}
            className="w-full text-left border border-slate-100 rounded-xl p-4 space-y-2 bg-white hover:border-slate-300 hover:shadow-sm transition"
        >
            ...
            {Array.isArray(review.mediaUrls) && review.mediaUrls.length > 0 && (
                <div className="flex flex-wrap gap-2 pt-1">
                    {review.mediaUrls.map((url, i) => (
                        <a key={i} href={url} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="w-16 h-16 rounded-md border border-slate-200 overflow-hidden bg-slate-50">
            ...
            <div className="flex items-center gap-2 pt-1 flex-wrap">
                <HandThumbsUp size={12} className="text-slate-400" />
                <span className="text-xs text-slate-400">{review.helpful} found this helpful</span>

                <button type="button" onClick={(e) => { e.stopPropagation(); onLike?.(review.id) }} className={`inline-flex items-center gap-1 px-2 py-1 rounded text-xs border ${liked ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-white text-slate-600 border-slate-200'}`}>
                    <HandThumbsUp size={11} />
                    Like {likedBy.length > 0 ? `(${likedBy.length})` : ''}
                </button>
                <button type="button" onClick={(e) => { e.stopPropagation(); onDislike?.(review.id) }} className={`inline-flex items-center gap-1 px-2 py-1 rounded text-xs border ${disliked ? 'bg-rose-50 text-rose-700 border-rose-200' : 'bg-white text-slate-600 border-slate-200'}`}>
                    <HandThumbsDown size={11} />
                    Dislike {dislikedBy.length > 0 ? `(${dislikedBy.length})` : ''}
                </button>
                {canDelete && (
                    <button type="button" onClick={(e) => { e.stopPropagation(); onDelete?.(review.id) }} className="inline-flex items-center gap-1 px-2 py-1 rounded text-xs border border-red-200 text-red-600 bg-red-50">
