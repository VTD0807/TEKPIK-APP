                ...
                <div className="flex items-center justify-between max-w-7xl mx-auto py-4 transition-all">
                    ...
                    <div className="hidden sm:flex items-center gap-4 lg:gap-6 text-slate-600">
                        ...
                        <Link href="/wishlist" className="relative text-slate-600 hover:text-red-500 transition">
                            <HeartIcon size={20} />
                            {wishlistCount > 0 && (
                                <span className="absolute -top-1.5 -right-1.5 text-[9px] text-white bg-red-500 w-4 h-4 rounded-full flex items-center justify-center font-bold">{wishlistCount}</span>
                            )}
                        </Link>

                        {/* Only show Admin button for admins */}
                        {isAdmin && (
                            <Link href="/admin" className="px-4 py-1.5 text-xs font-medium border border-indigo-300 hover:bg-indigo-50 transition text-indigo-600 rounded-full">
                                Admin
                            </Link>
                        )}
                            ...
                            <div className="relative" ref={menuRef}>
                                ...
                                {menuOpen && (
                                    <div className="absolute right-0 top-11 bg-white border border-slate-100 rounded-2xl shadow-xl py-2 w-52 z-50">
                                        ...
                                        <Link href="/wishlist" onClick={() => setMenuOpen(false)} className="flex items-center gap-2.5 px-4 py-2 text-sm text-slate-600 hover:bg-slate-50 transition">
                                            <HeartIcon size={14} /> Wishlist
                                        </Link>
                                        {isAdmin && (
                                            <Link href="/admin" onClick={() => setMenuOpen(false)} className="flex items-center gap-2.5 px-4 py-2 text-sm text-indigo-600 hover:bg-indigo-50 transition">
                                                <UserIcon size={14} /> Admin Panel
                                            </Link>
                                        )}
                                        <button onClick={handleSignOut} className="w-full flex items-center gap-2.5 px-4 py-2 text-sm text-red-500 hover:bg-red-50 transition">
                    ...
                    <div className="sm:hidden flex items-center gap-2">
                        ...
                        {isAdmin && (
                            <Link href="/admin" className="px-3 py-1.5 text-xs border border-indigo-300 text-indigo-600 rounded-full">Admin</Link>
                        )}
                        {user
