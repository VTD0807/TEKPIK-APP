'use client'
import { Search, HeartIcon, SparklesIcon, UserIcon, LogOutIcon } from "lucide-react"
            ...
            <div className="mx-6">
                <div className="flex items-center justify-between max-w-7xl mx-auto py-4 transition-all">
                    ...
                    <div className="hidden sm:flex items-center gap-4 lg:gap-6 text-slate-600">
                        ...
                        <Link href="/ai-picks" className="flex items-center gap-1 hover:text-indigo-600 transition text-sm"><SparklesIcon size={14} /> AI Picks</Link>
                        ...
                        {user ? (
                            <div className="relative" ref={menuRef}>
                                <button onClick={() => setMenuOpen(v => !v)} className="flex items-center gap-2 text-sm text-slate-700 hover:text-slate-900 transition">
                                    {avatar ? <img src={avatar} alt={name} referrerPolicy="no-referrer" className="w-8 h-8 rounded-full object-cover ring-2 ring-indigo-100" /> : <div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-semibold text-sm">{name[0]?.toUpperCase()}</div>}
                                ...
                                {menuOpen && (
                                    <div className="absolute right-0 top-11 bg-white border border-slate-100 rounded-2xl shadow-xl py-2 w-52 z-50">
                                        ...
                                        <Link href="/wishlist" onClick={() => setMenuOpen(false)} className="flex items-center gap-2.5 px-4 py-2 text-sm text-slate-600 hover:bg-slate-50 transition"><HeartIcon size={14} /> Wishlist</Link>
                                        {isAdmin && <Link href="/admin" onClick={() => setMenuOpen(false)} className="flex items-center gap-2.5 px-4 py-2 text-sm text-indigo-600 hover:bg-indigo-50 transition"><UserIcon size={14} /> Admin Panel</Link>}
                                        <button onClick={handleSignOut} className="w-full flex items-center gap-2.5 px-4 py-2 text-sm text-red-500 hover:bg-red-50 transition"><LogOutIcon size={14} /> Sign Out</button>
                                    </div>
                                )}
                            </div>
                        ) : (
                    ...
                    <div className="sm:hidden flex items-center gap-2">
                        <Link href="/wishlist" className="relative p-1.5"><HeartIcon size={20} className="text-slate-600" />{wishlistCount > 0 && <span className="absolute top-0 right-0 text-[8px] text-white bg-red-500 w-3.5 h-3.5 rounded-full flex items-center justify-center">{wishlistCount}</span>}</Link>
                        {isAdmin && <Link href="/admin" className="px-3 py-1.5 text-xs border border-indigo-300 text-indigo-600 rounded-full">Admin</Link>}
                        {user ? <button onClick={handleSignOut} className="flex items-center gap-1 px-3 py-1.5 text-xs text-red-500 border border-red-200 rounded-full"><LogOutIcon size={12} /> Out</button> : <Link href="/login" className="px-4 py-1.5 bg-indigo-500 text-xs text-white rounded-full font-medium">Login</Link>}
                    </div>
                </div>
            </div>
